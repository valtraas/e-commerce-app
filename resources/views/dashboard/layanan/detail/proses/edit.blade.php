...
<div class="mx-20 mb-20">
    <div class="border w-[35%] mx-auto p-6 rounded-xl shadow-xl mb-20">
        <form action="{{ route('proses.change',['proses'=>$proses->name]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
<div class="mb-5">
    <label for="name" class="block mb-3 dark:text-active font-bold text-xl">Name <span class="text-red-700">*</span></label>
    <input type="text" class="px-2 py-1 rounded-xl border border-active @error('name')ring-2 ring-red-700 border-none @enderror w-80" name="name" id="name" value="{{ $proses->name }}">
    @error('name')
...
<div class="mb-5">
    ...
    <textarea name="desc" id="" cols="35" rows="5 "class="px-2 py-1 rounded-xl border border-active @error('desc')ring-2 ring-red-700 border-none @enderror">{{ $proses->desc }}</textarea>
    @error('desc')
<div class="my-2 text-red-700 text-sm">
    {{ $message }}
</div>
...
<a href="{{ route('detail.index',['category'=>$category->slug]) }}" class="text-sm text-center block mt-6 dark:text-blue-700 text-active hover:underline hover:font-bold">Kembali</a>
</form>
    </div>
