...
<div class="h-16 border-b border-active flex items-center">
    <div class=" flex items-center px-4 dark:text-active  gap-4 text-2xl font-bold duration-300 delay-150">
        ...
        <p>Tambah Fitur Layanan {{ $layanan->name }}</p>
    </div>
...
<div class="mx-20 mb-20">
    <div class="border w-[35%] mx-auto p-6 rounded-xl shadow-xl mb-20">
        <form action="{{ route('fitur.ubah',['fitur'=>$fitur->name]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
<div class="mb-5">
    <label for="name" class="block mb-3 dark:text-active font-bold text-xl">Name <span class="text-red-700">*</span></label>
    <input type="text" class="px-2 py-1 rounded-xl border border-active @error('name')ring-2 ring-red-700 border-none @enderror w-80" name="name" id="name" value="{{ $fitur->name }}">
    @error('name')
...
<div class="mb-5">
    ...
    <textarea name="desc" id="" cols="35" rows="5 "class="px-2 py-1 rounded-xl border border-active @error('desc')ring-2 ring-red-700 border-none @enderror">{{ $fitur->desc }}</textarea>
    @error('desc')
<div class="my-2 text-red-700 text-sm">
    {{ $message }}
</div>
