...
<div class="mx-20 mb-20">
    <div class="border w-[25%] mx-auto p-6 rounded-xl shadow-xl mb-20">
        <form action="{{ route('teams.update',['team'=>$team->name]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
<div class="mb-5">
    <label for="name" class="block mb-3 dark:text-active font-bold text-xl">Name <span class="text-red-700">*</span></label>
    <input type="text" class="px-2 py-1 rounded-xl border border-active @error('name')ring-2 ring-red-700 border-none @enderror" name="name" id="name" value="{{ $team->name }}" >
    @error('name')
